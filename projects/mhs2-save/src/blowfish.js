...
/**
 * Generate Blowfish S-boxes and P-array from key
 */
function generateSBoxes(key) {
    ...
    const zeroBlock = new Uint8Array(8);
    let xl = 0, xr = 0;

    for (let i = 0; i < 18; i += 2) {
        const encrypted = this.encryptBlock(xl, xr, S, P);
        xl = encrypted[0];
        xr = encrypted[1];
        P[i] = xl;
        P[i + 1] = xr;
...
export class Blowfish {
    constructor(key) {
        ...
        const { S, P } = generateSBoxes(this.key);
        this.S = S;
        this.P = P;
    }
